...
Тогда получаем:
\[
L \cap R = \{ a^n b a^n b a^n b a b \mid n \ge 0 \}
\]

С помощью леммы о накачке для контекстно-свободных языков докажем, что $L\cap R$, а значит и исходный язык, не является КС.  

Пусть наше слово:
\[
w = a^p b a^p b a^p b a b
\]
...
\begin{enumerate}
    ...
    \item Накачивая отдельно правые $a$ или $b$, мы опять выходим из структуры языка.
    \item Накачивая соседние блоки, состоящие из $a^p$, вне зависимости от синхронности, мы также выходим из языка.
    \item Накачивая отдельно $b$ в промежутках выходим из языка.
\end{enumerate}

Следовательно, $L \cap R$ не контекстно-свободен, а значит и исходный язык $L$ не является контекстно-свободным.
